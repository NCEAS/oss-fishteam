...
envdataJunJul<-filter(envdata,month==6 | month==7)
moyrJJ<-filter(moyr,month==6 | month==7)

#envdataJJON<-dplyr::select(envdataJunJulOctNov,STATIONID,DECSLAT,DECSLON,year,month,day,TEMPMID,SALMID,CHLORSURF,OXYMAX)
#write.csv(envdataJJON,"envdataJJON.csv")

write.csv(envdataJunJul,"envdataNOnaJuneJuly.csv")

envdata1<-filter(envdataJunJul,!DECSLAT==-9999)
...
countr<-0
for (moyrl in sort(unique(moyrJJ$interaction))) {
  #print year-mo
  print(moyrl)
...
zip(zipfile = 'varsepenvshapezip', files = 'Enviroshp/varsep/')

####################################################################
#export shapefiles with joined jun/jul, by variable, without missing values in variable
#testloop
...
#############################################################################
## Didn't use this code, but may be useful later
